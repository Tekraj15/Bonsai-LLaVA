...
# Instead of Image -> SigLIP -> Projector -> LLM for each training step, 
# we split the pipeline into two steps:
# Preprocessing (Run Once): Image -> SigLIP -> Save Embeddings to Disk.
# Training (Run Many Times): Load Embeddings -> Projector -> LLM.
...
def precompute():
    ...
    # use the raw dataset class to handle image loading
    ...
    # 3. Processing Loop
    # store: input_ids, labels, attention_mask, and image_features (tensors)
    data_list = []
    
    print(f"Pre-computing features for {len(raw_dataset)} samples...")
    
    # iterate one by one (or small batch) to save memory
    with torch.no_grad():
